...
- **The settings accept only `true` or `false`**: in any case in the environment, as JSON booleans, without quotes, in `config.json`. `COOKIE_SECURE` and `auth.session.secure` also accept `auto`, their default. This holds for `OIDC_ENABLED`, `AUTH_REQUIRED`, `COOKIE_SECURE`, `auth.enabled`, `auth.session.secure` and `auth.backChannelLogout`, and for `TRUST_PROXY` and `RATE_LIMIT_ENABLED`, or `rateLimit.trustProxy` and `rateLimit.enabled` in `config.json`, which drive the CORS, `ALLOWED_HOSTS` and rate limiting checks: any other value stops the server at startup, naming the setting, and the file for `config.json`. `TRUST_PROXY=TRUE`, read as `false` before, now trusts the proxy.
