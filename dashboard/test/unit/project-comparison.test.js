...
// A row as [id, name, cost in month A, cost in month B, variation]
const rows = (compared) => compared
  .map(({ projectId, projectName, totalA, totalB, diff }) => (
    [projectId, projectName, totalA, totalB, diff]
  ));

describe('compareProjects', () => {
  it('pairs the projects billed in both months, with their variation from A to B', () => {
    expect(compareProjects([production(400), staging(200)], [production(500), staging(150)]))
      .toEqual([
        {
          projectId: 'project-production', projectName: 'Production',
          totalA: 400, totalB: 500, diff: 25,
        },
        {
          projectId: 'project-staging', projectName: 'Staging',
          totalA: 200, totalB: 150, diff: -25,
        },
      ]);
  });

  it('gives a project billed in month A only 0 € in month B, down 100 %', () => {
    expect(rows(compareProjects([production(400), staging(200)], [production(500)]))).toEqual([
  ...
  it('gives a project billed in month B only 0 € in month A, and no variation', () => {
    expect(rows(compareProjects([production(400)], [production(500), sandbox(120)]))).toEqual([
      ['project-production', 'Production', 400, 500, 25],
  ...
  it('gives the projects of month B when month A has none', () => {
    expect(rows(compareProjects([], [production(500), staging(150)]))).toEqual([
  ...
  it('gives no project when neither month has any', () => {
    expect(compareProjects([], [])).toEqual([]);
  });

  it('gives no variation to a project billed 0 € in month A', () => {
    expect(rows(compareProjects([production(0)], [production(120)])))
      .toEqual([['project-production', 'Production', 0, 120, null]]);
    expect(rows(compareProjects([production(0)], [production(0)])))
  ...
  it('keeps the projects of month A in their order, then those of month B only in theirs', () => {
    ...
    expect(rows(compareProjects(
      [staging(200), production(100)],
      [sandbox(300), production(500), archive],
  ...
  describe('a project without an id', () => {
    ...
    it('pairs by name, in either month, and takes the id of the other one if it has one', () => {
      expect(rows(compareProjects([legacy(100)], [project('project-legacy', 'Legacy', 150)])))
        .toEqual([['project-legacy', 'Legacy', 100, 150, 50]]);
      expect(rows(compareProjects([project('project-legacy', 'Legacy', 100)], [legacy(150)])))
        .toEqual([['project-legacy', 'Legacy', 100, 150, 50]]);
      expect(rows(compareProjects([legacy(100)], [legacy(150)])))
